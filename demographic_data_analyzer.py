...
def calculate_demographic_data(print_data=True):
    ...
    wealth_percentages = ((df[df["salary"] == ">50K"]["native-country"].value_counts() / df["native-country"].value_counts())*100).sort_values(ascending=False)
    highest_earning_country = wealth_percentages.index[0]
    highest_earning_country_percentage = round(wealth_percentages.iloc[0], 1)


    # Identify the most popular occupation for those who earn >50K in India.
    India = df[(df['native-country'] == 'India') & (df['salary'] == '>50K')]
    top_IN_occupation = (India['occupation'].value_counts()).sort_values(ascending=False).index[0]
